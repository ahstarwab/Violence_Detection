from torch import optim
from torch.optim import lr_scheduler
import adabound
import torch_optimizer as torch_optim
import torch
from torch import nn

def setup_solver(parameters, config):

    ''' 
    optimizer
    ''' 
    if config['optimizer']['name'] == 'Adam':
        optimizer = optim.Adam(parameters, lr=config['optimizer']['lr'], weight_decay=config['optimizer']['weight_decay'],
                                eps=config['optimizer']['eps'])
        #                       eps=1e-3 if args.fp16 else 1e-8)
    elif config['optimizer']['name'] == 'SGD':
        optimizer = optim.SGD(parameters, lr=config['optimizer']['lr'], weight_decay=config['optimizer']['weight_decay'],
                              momentum=0.9, nesterov=True)

    elif config['optimizer']['name'] == 'Adabound':
        optimizer = adabound.AdaBound(parameters, lr=config['optimizer']['lr'], final_lr=config['optimizer']['final_lr'])

    elif config['optimizer']['name'] == 'RAdam':
        optimizer = torch_optim.RAdam(parameters, lr=config['optimizer']['lr'], weight_decay=config['optimizer']['weight_decay'],
                                eps=config['optimizer']['eps'])

    elif config['optimizer']['name'] == 'Adagrad':
        optimizer = torch.optim.Adagrad(parameters, lr=config['optimizer']['lr'], weight_decay=config['optimizer']['weight_decay'], initial_accumulator=0)


    elif config['optimizer']['name'] == 'RMSprop':
        optimizer = torch.optim.RMSprop(parameters, lr=config['optimizer']['lr'], weight_decay=config['optimizer']['weight_decay'], alpha=0.99, eps=1e-08, momentum=0, centered=False)


    ''' 
    scheduler
    ''' 
    if config['scheduler']['name'] == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, patience=config['scheduler']['patience'], threshold=0.01, cooldown=0,
                                                   threshold_mode='abs', mode='max', factor=config['scheduler']['factor'])

    elif config['scheduler']['name'] == 'step':
        scheduler = lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.9, last_epoch=-1)

    elif config['scheduler']['name'] == 'exp':
        scheduler = lr_scheduler.ExponentialLR(optimizer, gamma=0.99)

    elif config['scheduler']['name'] == 'cycle':
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            optimizer,
            max_lr=0.001, 
            total_steps=10000,
            pct_start=0.3,
            base_momentum=0.9*0.95,
            max_momentum=0.95,
            final_div_factor=1/0.0001,
        )



    ''' 
    criterion
    ''' 
    if config['criterion']['name'] == 'CTCLoss':
        criterion = nn.CTCLoss(blank=2, zero_infinity=True)
    elif config['criterion']['name'] == 'CrossEntropy':
        criterion = nn.CrossEntropyLoss()

    return optimizer, scheduler, criterion

